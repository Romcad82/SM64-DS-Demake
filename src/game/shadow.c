...
Gfx *create_shadow_below_xyz(Vec3f pos, s16 shadowScale, u8 shadowSolidity, s8 shadowType, s8 shifted) {
    ...
    if (isEnvBox) {
		...
		//
		if (((obj->oVelX != 0) || (obj->oVelZ != 0)) && !(shifted) && (notHeldObj)) { // To fix objects w/ velocities causing shadow's normals to not align properly. Seems to work fine, but change if notice other objects break.
			struct Surface *intendedFloor;
			find_floor(obj->oPosX, obj->oPosY, obj->oPosZ, &intendedFloor);

			if ((floor != intendedFloor) && (intendedFloor != NULL)) {
				nx = intendedFloor->normal.x;
				ny = intendedFloor->normal.y;
				nz = intendedFloor->normal.z;
			}
		}
		//
