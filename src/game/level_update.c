...
void set_mario_initial_action(struct MarioState *m, u32 spawnType, u32 actionArg) {
    switch (spawnType) {
        ...
        case MARIO_SPAWN_SPIN_AIRBORNE:
			if (gCurrLevelNum == LEVEL_SI) { // This is for Sunshine Isles Spawn. This makes Mario stand still when spawning, instead of spinning in air for a frame.
				set_mario_action(m, ACT_IDLE, 0);
			} else {
				set_mario_action(m, ACT_SPAWN_SPIN_AIRBORNE, 0);
			}
			// Original: set_mario_action(m, ACT_SPAWN_SPIN_AIRBORNE, 0);
            break;
        ...
        case MARIO_SPAWN_LAUNCH_DEATH:
            ...
            break;
		case MARIO_SPAWN_CUSTOM_DOOR_WARP:
			set_mario_action(m, ACT_CUSTOM_WARP_DOOR_SPAWN, actionArg);
			break;
    }
...
void init_mario_after_warp(void) {
    ...
    switch (marioSpawnType) {
        ...
        case MARIO_SPAWN_FADE_FROM_BLACK:
            ...
            break;
		case MARIO_SPAWN_CUSTOM_DOOR_WARP:
			if (GET_BPARAM3(spawnNode->object->oBehParams) == 0x00) {
				play_transition(WARP_TRANSITION_FADE_FROM_CIRCLE, 0x10, 0x00, 0x00, 0x00);
			} else {
				play_transition(WARP_TRANSITION_FADE_FROM_COLOR, 0x1A, 0xFF, 0xFF, 0xFF);
			}
...
void check_instant_warp(void) {
 ...
 #else // !UNLOCK_ALL
    if (gCurrLevelNum == LEVEL_CASTLE
        && save_file_get_total_star_count(gCurrSaveFileNum - 1, COURSE_MIN - 1, COURSE_MAX - 1) >= 80) { // Infinite stairs
 #endif // !UNLOCK_ALL
        return;
    }
