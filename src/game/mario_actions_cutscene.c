...
static u8 sStarsNeededForDialog[] = { 1, 3, 8, 30, 50, 80, 12 };
...
s32 get_star_collection_dialog(struct MarioState *m) {
    ...
    for (i = 0; i < ARRAY_COUNT(sStarsNeededForDialog); i++) {
        ...
        if (m->prevNumStarsForDialog < numStarsRequired && m->numStars >= numStarsRequired) {
			...
			if (numStarsRequired == 12) {
				dialogID = DIALOG_180;
			} else {
				dialogID = i + DIALOG_141;
			}
            // Original: dialogID = i + DIALOG_141;
    ...
    if (m->actionState == ACT_STATE_STAR_DANCE_CUTSCENE) {
        switch (++m->actionTimer) {
            case 1:
				...
				} else if (obj_has_model(m->interactObj, MODEL_SILVER_STAR)) { // Issue with silver stars not setting model correctly? Used to happen a lot, but maybe fixed it accidentally? Most likely didn't fix source of problem.
					obj_set_model(celebStar, MODEL_SILVER_STAR);
				}
#endif
                disable_background_sound();
                //! TODO: Is this check necessary? Both seem to do the exact same thing.
                if (m->actionArg & 1) {
					if (obj_has_model(celebStar, MODEL_SILVER_STAR)) {
						play_silver_star_music(FALSE, m->interactObj->parentObj->oPlayUpdatedMusic);
					} else {
						// No exit
            ...
            case 80:
                if (!(m->actionArg & 1)) {
					...
					if (!(obj_has_model(cur_obj_nearest_object_with_behavior(bhvCelebrationStar), MODEL_SILVER_STAR))) { // This is dumb, but m->interactObj only seems to work sometimes? Maybe issue with Silver Stars being different interaction types than star or key?
						enable_time_stop();
						create_dialog_box_with_response(((gLastCompletedStarNum == 7) && (gCurrCourseNum != COURSE_NONE)) ? DIALOG_013 : DIALOG_014); // Grounds, Inside Castle, & Courtyard are all one course & there's 7 stars total between them all, so "(gCurrCourseNum != COURSE_NONE)" is needed to avoid 100 coin star dialogue
...
s32 act_custom_warp_door_spawn(struct MarioState *m) {
	struct Object *doorObj = NULL;
	if (GET_BPARAM1(m->usedObj->oBehParams) == 0x00) {
		doorObj = cur_obj_nearest_object_with_behavior(bhvDoorWarp);
	} else {
		doorObj = cur_obj_nearest_object_with_behavior(bhvDoor);
	}
    ...
    if (m->actionState == ACT_STATE_WARP_DOOR_SPAWN_SET_INTERACT_STATUS) {
        ...
        if (m->actionArg & WARP_FLAG_DOOR_PULLED) {
			doorObj->oInteractStatus = INT_STATUS_WARP_DOOR_PULLED;
        } else {
			doorObj->oInteractStatus = INT_STATUS_WARP_DOOR_PUSHED;
        }
    } else if (doorObj->oAction == DOOR_ACT_CLOSED) {
        set_mario_action(m, ACT_IDLE, 0);
    }
    set_mario_animation(m, MARIO_ANIM_FIRST_PERSON);
    stop_and_set_height_to_floor(m);
...
s32 mario_execute_cutscene_action(struct MarioState *m) {
    ...
    switch (m->action) {
		...
		case ACT_CUSTOM_WARP_DOOR_SPAWN:     cancel = act_custom_warp_door_spawn(m);     break;
