...
void calc_new_obj_vel_and_pos_y(struct Surface *objFloor, f32 objFloorY, f32 objVelX, f32 objVelZ) {
	...
	if (!(cur_obj_has_behavior(bhvChiefChilly))) {
		if (o->oVelY > 75.0) {
			o->oVelY = 75.0;
		}
	}
...
void spawn_orange_number(s8 behParam, s16 relX, s16 relY, s16 relZ) {
    ...
    if (behParam > (ORANGE_NUMBER_F + 6)) return;
#else
    if (behParam > (ORANGE_NUMBER_9 + 6)) return;
#endif
...
/**
 * Spawns an orange number object at a certain position.
 */
void spawn_orange_number_at_pos(s8 behParam, s16 x, s16 y, s16 z) {
#ifdef DIALOG_INDICATOR
	if (behParam > (ORANGE_NUMBER_F + 6)) return;
#else
	if (behParam > (ORANGE_NUMBER_9 + 6)) return;
	...
	struct Object* orangeNumber = spawn_object_relative(behParam, x, y, z, o, MODEL_NUMBER, bhvOrangeNumber);
	orangeNumber->oPosY = y;
	orangeNumber->oPosY += 25.0f;
	orangeNumber->oOrangeNumberOffset = 0;
	orangeNumber->oHomeX = x;
	orangeNumber->oHomeZ = z;
}
...
#include "behaviors/spawn_object_from_another.inc.c"
#include "behaviors/spawn_object_with_coordinates.inc.c"
