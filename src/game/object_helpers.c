...
static s8 sLevelsWithRooms[] = { LEVEL_BBH, LEVEL_CASTLE, LEVEL_HMC, LEVEL_BBB, -1 };
...
s32 cur_obj_rotate_roll_toward(s16 target, s16 increment) {
	...
	startRoll = (s16) o->oMoveAngleRoll;
	o->oMoveAngleRoll = approach_s16_symmetric(o->oMoveAngleRoll, target, increment);

	if ((o->oAngleVelRoll = (s16)((s16)o->oMoveAngleRoll - startRoll)) == 0) {
		return TRUE;
	} else {
		return FALSE;
	}
...
void set_action_for_all_objects_with_behavior(const BehaviorScript *behavior, u8 action) {
	uintptr_t *behaviorAddr = segmented_to_virtual(behavior);
	struct ObjectNode *listHead = &gObjectLists[get_object_list_from_behavior(behaviorAddr)];
	struct Object *obj = (struct Object *) listHead->next;
...
void set_action_for_all_objects_with_behavior_not_dead(const BehaviorScript *behavior, u8 action) {
	uintptr_t *behaviorAddr = segmented_to_virtual(behavior);
	struct ObjectNode *listHead = &gObjectLists[get_object_list_from_behavior(behaviorAddr)];
	struct Object *obj = (struct Object *) listHead->next;

	while (obj != (struct Object *) listHead) {
		if ((obj->behavior == behaviorAddr) && !((obj->oAction >= OBJ_ACT_HORIZONTAL_KNOCKBACK) && (obj->oAction <= OBJ_ACT_SQUISHED))) {
			obj->oAction = action;
		}
...
static void cur_obj_move_xz(f32 steepSlopeNormalY, s32 careAboutEdgesAndSteepSlopes) { // something with cur_obj_move_xz fucks up shadows on slopes
    ...
    if (!(o->oMoveFlags & OBJ_MOVE_MASK_IN_WATER)) {
        ...
        if (o->oPosY < waterLevel) {
            ...
            if (o->oPosY < o->oFloorHeight) {
				...
				if (!(cur_obj_has_behavior(bhvSilverStar))) {
					o->oPosY = waterLevel;
					o->oVelY = 0.0f;
				}
				/* Original:
...
void obj_spawn_particles(struct SpawnParticlesInfo *info, struct Object *obj) {
    struct Object *particle;
    s32 i;
    f32 scale;
    s32 numParticles = info->count;
    ...
    for (i = 0; i < numParticles; i++) {
        ...
        particle = spawn_object(obj, info->model, bhvWhitePuffExplosion);
...
Gfx *geo_offset_klepto_held_silver_star(s32 callContext, struct GraphNode *node, UNUSED Mat4 mtx) { // This is dumb, but I'm not smart enough to know another way to achieve the same result
    if (callContext == GEO_CONTEXT_RENDER) {
        vec3s_set(((struct GraphNodeTranslationRotation *) node->next)->translation, 300, 450, 0);
    }
...
s32 mario_is_within_rectangle(f32 minX, f32 maxX, f32 minZ, f32 maxZ) {
...
s32 cur_obj_is_within_rectangle(f32 minX, f32 maxX, f32 minZ, f32 maxZ) {
	if (o->oPosX < minX || maxX < o->oPosX) {
		return FALSE;
	}

	if (o->oPosZ < minZ || maxZ < o->oPosZ) {
		return FALSE;
	}
...
s32 mario_inbetween_cur_obj_angles(s32 minAngle, s32 maxAngle) {
	s32 marioAngle = o->oAngleToMario;
	if (minAngle < 0x0000) {
		minAngle += 0x10000;
	}
	if (maxAngle < 0x0000) {
	...
	if (marioAngle < 0x0000) {
		marioAngle += 0x10000;
	}

	if ((marioAngle >= minAngle) && (marioAngle <= maxAngle)) {
		return TRUE;
	} else {
		return FALSE;
	}
...
s32 cur_obj_inbetween_obj_angles(struct Object *obj, s32 minAngle, s32 maxAngle) {
	s32 objAngle = obj_angle_to_object(obj, o);
	if (minAngle < 0x0000) {
		minAngle += 0x10000;
	}
	if (maxAngle < 0x0000) {
	...
	if ((objAngle >= minAngle) && (objAngle <= maxAngle)) {
		return TRUE;
	} else {
		return FALSE;
	}
...
s32 obj_inbetween_cur_obj_angles(struct Object *obj, s32 minAngle, s32 maxAngle) {
	s32 objAngle = obj_angle_to_object(o, obj);
	if (minAngle < 0x0000) {
		minAngle += 0x10000;
	}
	if (maxAngle < 0x0000) {
...
void cur_obj_spawn_star_at_all_offsets(f32 targetX, f32 targetY, f32 targetZ, f32 offsetX, f32 offsetY, f32 offsetZ) {
	f32 objectPosX = o->oPosX;
	f32 objectPosY = o->oPosY;
	f32 objectPosZ = o->oPosZ;
	o->oPosX += offsetX + gDebugInfo[DEBUG_PAGE_ENEMYINFO][0];
	o->oPosY += offsetY + gDebugInfo[DEBUG_PAGE_ENEMYINFO][0];
	o->oPosZ += offsetZ + gDebugInfo[DEBUG_PAGE_ENEMYINFO][0];
	spawn_default_star(targetX, targetY, targetZ);
	o->oPosX = objectPosX;
	o->oPosY = objectPosY;
	o->oPosZ = objectPosZ;
