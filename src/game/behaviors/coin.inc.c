...
void coin_inside_boo_act_dropped(void) {
	...
	if (cur_obj_has_model(MODEL_RED_COIN)) {
		struct Object *redCoin = spawn_object(o, MODEL_RED_COIN, bhvRedCoin);
		redCoin->oBehParams2ndByte = 0x01;
		redCoin->oWallHitboxRadius = o->oWallHitboxRadius;
		redCoin->oGravity = o->oGravity;
		redCoin->oBounciness = o->oBounciness;
		redCoin->oDragStrength = o->oDragStrength;
		redCoin->oFriction = o->oFriction;
		redCoin->oBuoyancy = o->oBuoyancy;
		vec3f_copy(&redCoin->oVelVec, &o->oVelVec);
		obj_mark_for_deletion(o);
	}
...
void coin_inside_boo_act_carried(void) {
    ...
    cur_obj_become_intangible();
    if (o->oTimer == 0 && GET_BPARAM1(parent->oBehParams) == COIN_INSIDE_BOO_BP_BLUE_COIN) {
        cur_obj_set_model(MODEL_BLUE_COIN);
        cur_obj_scale(0.7f);
	} else if (o->oTimer == 0 && GET_BPARAM1(parent->oBehParams) == COIN_INSIDE_BOO_BP_RED_COIN) {
