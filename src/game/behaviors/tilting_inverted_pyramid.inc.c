...
void bhv_tilting_inverted_pyramid_loop(void) {
	...
	if ((gCurrLevelNum == LEVEL_GB) && (o->oBehParams2ndByte == 0x01)) {
		mtxf_align_terrain_normal(*transform, &o->oTiltingPyramidNormalVec, &o->oPosVec, 0x2000);
	} else {
		mtxf_align_terrain_normal(*transform, &o->oTiltingPyramidNormalVec, &o->oPosVec, 0x0);
	}
    // Original: mtxf_align_terrain_normal(*transform, &o->oTiltingPyramidNormalVec, &o->oPosVec, 0x0);
