...
struct ExclamationBoxContents sExclamationBoxContents[] = {
	...
	{ EXCLAMATION_BOX_BP_COINS_5,          0, 0, MODEL_NONE,             bhvFiveCoinsSpawn        },
...
void exclamation_box_spawn_contents(struct ExclamationBoxContents *contentsList, u8 boxType) {
    ...
    while (contentsList->id != EXCLAMATION_BOX_BP_NULL) {
        if (boxType == contentsList->id) {
			//
			if ((o->oBehParams2ndByte <= EXCLAMATION_BOX_BP_VANISH_CAP) && !(gMarioState->flags & (MARIO_CAP_ON_HEAD | MARIO_CAP_IN_HAND))) {
				contentsObj = spawn_object(o, MODEL_BLACK_BOBOMB, bhvLitBobomb);
			} else {
				contentsObj = spawn_object(o, contentsList->model, contentsList->behavior);
			}
            // Original: contentsObj = spawn_object(o, contentsList->model, contentsList->behavior);
