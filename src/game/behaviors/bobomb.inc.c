...
void bhv_lit_bobomb_init(void) {
	...
	switch (GET_BPARAM4(o->oBehParams)) {
		case 0x01:
			...
			o->oSOWCObj = cur_obj_nearest_object_with_behavior(bhvSpawnObjectWithCoordinates);
			break;

		case 0x02:
			o->oPosY = 1500.0f;
			o->oMoveAngleYaw = 0xD555;
		...
		case 0x03:
			o->oPosY = 1500.0f;
			o->oMoveAngleYaw = 0x6AAB;

			o->oSOWCObj = cur_obj_nearest_object_with_behavior(bhvSpawnObjectWithCoordinates);
			break;

		default:
			o->oPosY = cur_obj_nearest_object_with_behavior(bhvExclamationBox)->oPosY;
			break;
	}
...
void bobomb_act_explode(void) {
    if (o->oTimer < 5) {
		...
		if (!cur_obj_has_behavior(bhvLitBobomb)) {
        ...
        // Original: create_respawner(MODEL_BLACK_BOBOMB, bhvBobomb, 3000);
...
void bhv_bobomb_loop(void) {
	...
	} else if (cur_obj_has_behavior(bhvLitBobomb)) {
		if (o->oSOWCObj != NULL) {
			o->oSOWCObj->oAction--;
		}
