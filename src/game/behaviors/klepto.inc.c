...
static Vec3f sKleptoSITargetPositions[] = {
	{ 1366.0f, 300.0f, -2728.0f },
	{ -1310.0f, 1600.0f, -3201.0f },
	{ -2542.0f, 300.0f, 446.0f },
	{ 2258.0f, 300.0f, 948.0f },
};
...
void bhv_klepto_init(void) {
    if (o->oBehParams2ndByte == KLEPTO_BP_HAS_STAR) { // Original: if (o->oBehParams2ndByte != KLEPTO_BP_NO_STAR)
    ...
    } else if (o->oBehParams2ndByte == KLEPTO_BP_NO_STAR) { // Original: } else {
...
static void klepto_change_target(void) {
    ...
    if (o->oDistanceToMario > 2000.0f) {
        ...
        for (i = 0; i <= 3; i++) {
			...
			if (gCurrCourseNum == 8) {
				...
				dx = gMarioObject->oPosX - sKleptoSITargetPositions[i][0];
				dz = gMarioObject->oPosZ - sKleptoSITargetPositions[i][2];
			}
			/* Original:
            dx = gMarioObject->oPosX - sKleptoTargetPositions[i][0];
            dz = gMarioObject->oPosZ - sKleptoTargetPositions[i][2];
	...
	if (gCurrCourseNum == 8) {
		...
		vec3f_copy_y_off(&o->oHomeVec, sKleptoSITargetPositions[o->oKleptoTargetNumber], o->oKleptoHomeYOffset);
	}
    // Original: vec3f_copy_y_off(&o->oHomeVec, sKleptoTargetPositions[o->oKleptoTargetNumber], o->oKleptoHomeYOffset);
...
static void klepto_act_dive_at_mario(void) {
    ...
    if (o->oSoundStateID == KLEPTO_ANIM_DIVE_AT_MARIO_2) {
        ...
        if (o->oAnimState == KLEPTO_ANIM_STATE_HOLDING_NOTHING) {
			...
			// If Klepto doesn't have Silver Star, then let him steal Mario's cap
			if (o->oBehParams2ndByte != KLEPTO_BP_SILVER_STAR) {
			...
			// Else, set his damage value to 1
			} else {
    ...
    if (o->oAction == KLEPTO_ACT_STRUCK_BY_MARIO) {
        ...
        if (obj_handle_attacks(&sKleptoHitbox, o->oAction, sKleptoAttackHandlers)) {
            ...
            if (o->oAnimState == KLEPTO_ANIM_STATE_HOLDING_CAP) {
				...
				silverStar->oFaceAngleRoll = 0x0000;
			}
