    ...
    if (!(o->activeFlags & ACTIVE_FLAG_IN_DIFFERENT_ROOM)) {
        ...
        if (o->oBehParams2ndByte != CAMERA_LAKITU_BP_FOLLOW_CAMERA) {
			...
			if (gCurrLevelNum == LEVEL_BBB) {
				mirroredX = BBB_MIRROR_X - gLakituState.curPos[0];
			} else {
				mirroredX = CASTLE_MIRROR_X - gLakituState.curPos[0];
			}
            // Original: f32 mirroredX = CASTLE_MIRROR_X - gLakituState.curPos[0];

            if ((gLakituState.curPos[0] < 1600.0f) && (gCurrLevelNum != LEVEL_BBB)) { // Original: if (gLakituState.curPos[0] < 1700.0f || mirroredX < 0.0f)
                cur_obj_hide();
            } else {
                cur_obj_unhide();
				...
				if (gCurrLevelNum == LEVEL_BBB) {
					...
					if ((gMarioObject->oPosX > 4231.0f) && (o->oPlayedJingle == FALSE)) {
						play_puzzle_jingle();
						o->oPlayedJingle = TRUE;
					}
				}
                // Original: o->oPosX = CASTLE_MIRROR_X + mirroredX;
