...
#define SPAWN_CASTLE_BOO_STAR_REQUIREMENT 15
#define SPAWN_RED_COIN_BOO_STAR_REQUIREMENT 3
...
static struct ObjectHitbox sKingBooHitbox = {
	/* interactType:      */ 0,
	/* downOffset:        */ 0,
	/* damageOrCoinValue: */ 3,
	/* health:            */ 3,
...
void bhv_king_boo_init(void) {
	o->oBooInitialMoveYaw = o->oMoveAngleYaw;
	o->oKingBooInvisible = 1;
	o->oKingBooTimer = 0;
	o->oAction = 7;
}

void bhv_boo_mirror_init(void) {
...
void bhv_courtyard_boo_triplet_init(void) {
    s32 i;
//#ifndef UNLOCK_ALL
    if ((gHudDisplay.stars < SPAWN_CASTLE_BOO_STAR_REQUIREMENT) || !(save_file_get_star_flags(gCurrSaveFileNum - 1, COURSE_NUM_TO_INDEX(gCurrCourseNum)) & STAR_FLAG_ACT_100_COINS)) {
...
//#endif
	{
		struct Object* boo = spawn_object(o, MODEL_BOO, bhvGhostHuntBoo);
		OR_BPARAM1(boo->oBehParams, COIN_INSIDE_BOO_BP_RED_COIN);
		boo->oBehParams2ndByte = BOO_BP_NORMAL;
	}
}
...
static s32 big_boo_update_during_nonlethal_hit(f32 a0) {
    ...
    if (o->oTimer < 32) {
        ...
        // Original: cur_obj_become_tangible();
...
static s32 boo_update_during_death(void) {
    if (o->oTimer == 0) {
        ...
        if (o->oTimer > 30 || o->oMoveFlags & OBJ_MOVE_HIT_WALL) {
			if ((o->oTimer == 31 || o->oMoveFlags & OBJ_MOVE_HIT_WALL) && (o->oBooDeathStatus != BOO_DEATH_STATUS_DEAD)) {
				spawn_mist_particles();
			}
            // Original: spawn_mist_particles();
        ...
        if (boo_update_during_death()) {
			if (cur_obj_has_behavior(bhvKingBoo)) {
				...
				if (cur_obj_update_dialog_with_cutscene(MARIO_DIALOG_LOOK_UP,
					DIALOG_FLAG_TURN_TO_MARIO, CUTSCENE_DIALOG, DIALOG_184)) {
					cur_obj_disable();
					o->oAction = 4;
...
static void king_boo_disappear_and_reappear_act_5(void) {
	boo_stop();

	if (o->oTimer < 90) {
		o->oBooTargetOpacity = 0;
		cur_obj_become_intangible();
		if ((o->oOpacity == 0) && (cur_obj_nearest_object_with_behavior(bhvKingBooMirror)->oOpacity == 0)) {
			cur_obj_hide();
		}
	}
	else if (o->oTimer < 104) {
		if (o->oTimer == 90) {
			o->oPosX = -((s32)random_u16() % 1850);
			o->oPosZ = ((s32)random_u16() % 1350) * random_sign();
...
static void king_boo_set_attack_status(void) {
	s32 attackStatus = boo_get_attack_status();
...
static void king_boo_spit_fire_act_6(void) {
	s16 yawIncrement;
	...
	cur_obj_rotate_yaw_toward(cur_obj_angle_to_home(), yawIncrement);
	...
	if ((random_u16() % 50 == 0) || (o->oTimer >= 60)) {
		o->oAction = 8;
	}

	king_boo_set_attack_status();
}

// Note: The trigger for King Boo fight is in King Boo Mirror object. This is so Mario faces the mirror version of King Boo.
static void king_boo_start_act_7(void) {
	o->oForwardVel = 0.0f;
	o->oBooTargetOpacity = 0;
	o->oKingBooTimer = 0;
	cur_obj_hide();
...
static void king_boo_wait_act_8(void) {
	s16 yawIncrement;
	if (o->oHealth == 3) {
		yawIncrement = 0x180;
	} else if (o->oHealth == 2) {
...
static ObjActionFunc sKingBooActions[] = {
	big_boo_act_0,
	big_boo_act_1,
	big_boo_act_2,
	big_boo_act_3,
	big_boo_act_4,
	king_boo_disappear_and_reappear_act_5,
	king_boo_spit_fire_act_6,
	king_boo_start_act_7,
	king_boo_wait_act_8,
};
...
void bhv_king_boo_loop(void) {
	//PARTIAL_UPDATE

	// glitch where king boo doesn't turn (visual glitch). fixed?
	// find way to get camera to zoom into mario when king boo first appears

	obj_set_hitbox(o, &sKingBooHitbox);

	o->oGraphYOffset = o->oBooBaseScale * 60.0f;

	cur_obj_update_floor_and_walls();
	cur_obj_call_action_function(sKingBooActions);
	cur_obj_move_standard(78);

	boo_approach_target_opacity_and_update_scale();
	o->oInteractStatus = INT_STATUS_NONE;

	// For King Boo to turn invisible
	if ((((o->oKingBooTimer >= 450) && ((random_u16() % 100) == 0)) || (o->oKingBooTimer >= 600)) && (o->oAction == 1)) {
		o->oAction = 5;
	}
	// For King Boo to spit fire
	else if ((random_u16() % 100 == 0) && (o->oOpacity == 255) && (o->oTimer > 30) && (o->oAction == 1)) {
		o->oAction = 6;
	}
	...
	if (o->oKingBooInvisible > 0) {
		o->oOpacity = 0;
		obj_copy_scale(o, cur_obj_nearest_object_with_behavior(bhvKingBooMirror));
	}

	// Prevent King Boo clipping into mirror
	if (o->oHealth == 3) {
		if (o->oPosX > -210.0f) {
			o->oPosX = -210.0f;
		}
...
void bhv_king_boo_mirror_loop(void) {
	f32 mirroredX;
	mirroredX = BBB_MIRROR_X - o->parentObj->oPosX;

	o->oBooBaseScale = o->parentObj->oBooBaseScale;
	o->oGraphYOffset = o->parentObj->oBooBaseScale * 60.0f;
	...
	if (cur_obj_nearest_object_with_behavior(bhvKingBoo) != o->parentObj) {
		...
		o->oPosX = BBB_MIRROR_X + mirroredX;
	}

	if (o->parentObj->oKingBooInvisible > 0) {
		o->oBooTargetOpacity = o->parentObj->oBooTargetOpacity;
		boo_approach_target_opacity_and_update_scale();
		boo_oscillate(FALSE);
	} else {
		o->oOpacity = 0;
	...
	if ((o->parentObj->header.gfx.node.flags & GRAPH_RENDER_INVISIBLE) == 0) {
		cur_obj_unhide();
	} else {
		cur_obj_hide();
	}

	if ((o->parentObj->oBooDeathStatus == BOO_DEATH_STATUS_DEAD) && (o->oBooDeathStatus != BOO_DEATH_STATUS_DEAD)) {
		spawn_mist_particles();
		o->oBooDeathStatus = BOO_DEATH_STATUS_DEAD;
	}
...
void bhv_boo_in_castle_loop(void) {
	...
	if (gHudDisplay.stars >= SPAWN_CASTLE_BOO_STAR_REQUIREMENT) {
		o->oBooBaseScale = 2.0f;
	} else if (gHudDisplay.stars >= SPAWN_RED_COIN_BOO_STAR_REQUIREMENT) {
		o->oBooBaseScale = 1.0f;
	}
    // Original: o->oBooBaseScale = 2.0f;

    if (o->oAction == 0) {
        cur_obj_hide();
#ifndef UNLOCK_ALL
        if (gHudDisplay.stars < SPAWN_RED_COIN_BOO_STAR_REQUIREMENT) {
            obj_mark_for_deletion(o);
        }
#endif
