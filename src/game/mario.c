            ...
            switch (floorType) {
				...
				case SURFACE_HORIZONTAL_WIND:
					if (gCurrLevelNum == LEVEL_CCC) {
						floorSoundType = 3;
					} // May cause crash or break if floorSoundType not initiliazed? Testing shows it doesn't, but just in case it does in future, then come back and init it here
					break;

				case SURFACE_PAINTING_WOBBLE_A6:
				case SURFACE_PAINTING_WOBBLE_A7:
				case SURFACE_PAINTING_WOBBLE_A8:
					if ((gCurrLevelNum == LEVEL_CASTLE) && (gCurrAreaIndex == 4)) {
						floorSoundType = 4;
					} // Same as above
					break;

				case SURFACE_PAINTING_WOBBLE_AC:
				case SURFACE_PAINTING_WOBBLE_AD:
				case SURFACE_PAINTING_WOBBLE_AE:
