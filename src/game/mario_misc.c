...
Gfx *geo_render_mirror_mario(s32 callContext, struct GraphNode *node, UNUSED Mat4 *mtx) {
    ...
    switch (callContext) {
        ...
        case GEO_CONTEXT_RENDER:
            if ((mario->header.gfx.pos[0] > 1700.0f) || (gCurrLevelNum == LEVEL_BBB)) { // Original: if (mario->header.gfx.pos[0] > 1700.0f)
				...
				if (gCurrLevelNum == LEVEL_BBB) {
					mirroredX = BBB_MIRROR_X - gMirrorMario.pos[0];
					gMirrorMario.pos[0] = mirroredX + BBB_MIRROR_X;
				} else {
...
Gfx *geo_mirror_mario_backface_culling(s32 callContext, struct GraphNode *node, UNUSED Mat4 *mtx) {
    ...
    if (callContext == GEO_CONTEXT_RENDER && gCurGraphNodeObject == &gMirrorMario) {
        ...
        if (asGenerated->parameter == 0) {	// Inverted
		...
		if (gMarioState->marioBodyState->modelState & MODEL_STATE_ALPHA) { // When Mario has Vanish Cap, normals of Mirror Mario get inverted. Setting node layer to transparent while Mario has the alpha effect applied to him fixes this.
        ...
        // Original: SET_GRAPH_NODE_LAYER(asGenerated->fnNode.node.flags, LAYER_OPAQUE);
