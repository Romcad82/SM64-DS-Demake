...
s32 update_parallel_tracking_camera(struct Camera *c, Vec3f focus, Vec3f pos) {
	...
	switch (gCurrLevelArea) {
		case AREA_CASTLE_TIPPY:
			if (c->pos[2] < path[0][2]) {
				sParTrackTransOff.pos[2] = path[0][2] - c->pos[2];
			} else if (c->pos[2] > path[1][2]) {
				sParTrackTransOff.pos[2] = path[1][2] - c->pos[2];
			}
            break;
...
s32 update_fixed_camera(struct Camera *c, Vec3f focus, UNUSED Vec3f pos) {
    ...
    switch (gCurrLevelArea) {
		...
		case AREA_BBB:
			if (mario_is_within_rectangle(-300.0f, 300.0f, 1200.0f, 2100.0f)) {
				scaleToMario = 0.3f;
			} else {
				scaleToMario = 0.f;
			}
	...
	// Manually set the camera height because the code above does not fucking work. When I set the height to a specific number, I expect it to be that number, not a random number at least 600 units above where it should be.
	switch (gCurrLevelArea) {
		case AREA_CASTLE_TIPPY:
			goalHeight = 1532.0f;
			break;
		...
		case AREA_BBH:
			if (((c->pos[0] == 3493) && (c->pos[2] == 617)) && (goalHeight > 1950.0f)) {
				goalHeight = 1950.0f;
			}
			break;
...
struct ParallelTrackingPoint sBBBBossParTrackPath[] = {
	{ 1, { -1999.99f, 274.902f, -15.0f }, 50.0f, 0.0f },
	{ 0, { -1999.99f, 274.902f, 15.0f }, 50.0f, 0.0f },
};

struct ParallelTrackingPoint sCastleMirrorTrackPath[] = {
	{ 1, { 3732.0f, 1908.0f, 1223.56f }, 100.0f, 0.0f },
	{ 0, { 3732.0f, 1908.0f, 2383.56f }, 100.0f, 0.0f },
};
...
u8 get_cutscene_from_mario_status(struct Camera *c) {
    ...
    if (cutscene == CUTSCENE_NONE) {
        ...
        if (sMarioCamState->cameraEvent == CAM_EVENT_DOOR) {
            switch (gCurrLevelArea) {
                case AREA_CASTLE_LOBBY:
					if (mario_is_within_rectangle(344, 944, -1776, -1176)) {
...
/**
 * This exists so transitioning to parallel tracking mode doesn't crash game if values aren't set
 */
void parallel_tracking_only_init_path(struct ParallelTrackingPoint* path) {
	sParTrackPath = path;
	sParTrackIndex = 0;
	sParTrackTransOff.pos[0] = 0.f;
	sParTrackTransOff.pos[1] = 0.f;
...
void cam_castle_rec_room_2(struct Camera *c) {
	if ((gMarioObject->oPosX < 2964.0f) && (gMarioObject->oPosZ > -3779.0f)) {
		c->doorStatus = DOOR_LEAVING_SPECIAL;
	}
	set_camera_mode_fixed(c, 3090, 1150, -2950);
}

void cam_castle_rec_room_3(struct Camera *c) {
	set_camera_mode_fixed(c, 3865, 1250, -4478);
}

void cam_castle_enter_mirror(struct Camera *c) {
	c->doorStatus = DOOR_LEAVING_SPECIAL;
	if (c->mode == CAMERA_MODE_CLOSE) {
		parallel_tracking_only_init_path(sCastleMirrorTrackPath);
		transition_to_camera_mode(c, CAMERA_MODE_PARALLEL_TRACKING, 20);
	} else {
		parallel_tracking_init(c, sCastleMirrorTrackPath);
	}
}

void cam_castle_exit_mirror(struct Camera *c) {
	transition_to_camera_mode(c, CAMERA_MODE_CLOSE, 20);
}

void cam_castle_white_room(struct Camera *c) {
	c->doorStatus = DOOR_LEAVING_SPECIAL;
	set_camera_mode_fixed(c, 9022, 1532, 2309);
}
...
void cam_gb_free_roam(struct Camera *c) {
	if (c->mode != CAMERA_MODE_FREE_ROAM) {
		transition_to_camera_mode(c, CAMERA_MODE_FREE_ROAM, 20);
	}
}

void cam_gb_walljump(struct Camera *c) {
	if (c->mode != CAMERA_MODE_8_DIRECTIONS) {
		transition_to_camera_mode(c, CAMERA_MODE_8_DIRECTIONS, 20);
		s8DirModeYawOffset = c->yaw;
	}
}

void cam_gb_fixed(struct Camera* c) {
	vec3f_set(sFixedModeBasePosition, 1088, 5450, 2600);
	transition_to_camera_mode(c, CAMERA_MODE_FIXED, 20);
}

void cam_bbb_free_roam(struct Camera* c) {
	if (c->mode != CAMERA_MODE_FREE_ROAM) {
		transition_to_camera_mode(c, CAMERA_MODE_FREE_ROAM, 20);
	}
}

void cam_bbb_fixed_1(struct Camera* c) {
	vec3f_set(sFixedModeBasePosition, 196, 300, 500);
	transition_to_camera_mode(c, CAMERA_MODE_FIXED, 20);
}

void cam_bbb_fixed_2(struct Camera* c) {
	vec3f_set(sFixedModeBasePosition, 6700, 450, -11000);
	transition_to_camera_mode(c, CAMERA_MODE_FIXED, 20);
}

void cam_bbb_area_2(struct Camera *c) {
	parallel_tracking_init(c, sBBBBossParTrackPath);
}

void cam_ccc_free_roam(struct Camera* c) {
	if (c->mode != CAMERA_MODE_FREE_ROAM) {
		transition_to_camera_mode(c, CAMERA_MODE_FREE_ROAM, 20);
	}
}

void cam_ccc_fixed(struct Camera* c) {
	vec3f_set(sFixedModeBasePosition, -7308, -3400, 3332);
	transition_to_camera_mode(c, CAMERA_MODE_FIXED, 20);
}
...
struct CameraTrigger sCamCastle[] = {
	...
	{ 1, cam_castle_close_mode, 4021, 307, -230, 140, 150, 140, 0x4000 },
	...
	{ 4, cam_castle_rec_room_2, 2714, 600, -3528, 300, 200, 300, 0xdfff },
	{ 4, cam_castle_rec_room_2, 2722, 600, -4263, 200, 200, 200, 0xdfff },
	{ 4, cam_castle_rec_room_2, 3894, 600, -3971, 200, 200, 200, 0xdfff },
	{ 4, cam_castle_rec_room_3, 4247, 600, -4325, 300, 200, 300, 0xdfff },
	{ 4, cam_castle_rec_room_3, 4797, 745, -4274, 200, 150, 200, 0xdfff },
	{ 2, cam_castle_exit_mirror, 4232, 1908, 1828, 100, 600, 1700, 0 },
	{ 2, cam_castle_enter_mirror, 4432, 1908, 1828, 100, 600, 1700, 0 },
	{ 2, cam_castle_enter_mirror, 6794, 1203, 1828, 200, 200, 200, 0 },
	{ 2, cam_castle_white_room, 7194, 1203, 1828, 200, 200, 200, 0 },
    NULL_TRIGGER
};
...
struct CameraTrigger sCamBBH[] = {
	...
	{ 1, cam_bbh_room_9_attic, 1787, 1886, 973, 200, 150, 200, 0 },
    NULL_TRIGGER
};
 ...
 */
struct CameraTrigger sCamTsobf[] = {
	NULL_TRIGGER
};
...
struct CameraTrigger sCamGb[] = {
	...
	{1, cam_gb_fixed, 2152, 5250, 2600, 200, 400, 300, 0xffff},
	{1, cam_gb_free_roam, 2900, 5500, 6100, 100, 1000, 500, 0xffff},
	{1, cam_gb_free_roam, 1752, 5250, 2600, 200, 400, 700, 0xffff},
	{1, cam_gb_free_roam, 2552, 5250, 2600, 200, 400, 700, 0xffff},
	{1, cam_gb_free_roam, 2152, 5250, 3100, 200, 400, 200, 0xffff},
	{1, cam_gb_free_roam, 2152, 5250, 2100, 200, 400, 200, 0xffff},
...
struct CameraTrigger sCamBbb[] = {
	{1, cam_bbb_fixed_1, 0, 400, 1700, 400, 500, 500, 0xffff},
	{1, cam_bbb_free_roam, 0, 400, 1100, 400, 500, 100, 0xffff},
	{1, cam_bbb_fixed_2, 7000, 400, -11000, 400, 500, 100, 0xffff},
	{1, cam_bbb_free_roam, 7000, 400, -10800, 400, 500, 100, 0xffff},
	{2, cam_bbb_area_2, -1500, 1700, 0, 100, 100, 100, 0xffff},
	NULL_TRIGGER
};
 ...
 */
void cutscene_non_painting_set_cam_pos(struct Camera *c) { // Manual Camera Adjustment
    struct Surface *floor;
    ...
    switch (gPrevLevel) {
		...
		case LEVEL_TSOBF:
			vec3f_set(c->pos, 2867.f, 205.f, -1740.f);
			break;

		case LEVEL_GB:
			vec3f_set(c->pos, 2800.f, 7800.f, -4950.f);
			break;

		case LEVEL_BBB:
			if (gCurrLevelNum != LEVEL_CASTLE_COURTYARD) {
				vec3f_set(c->pos, 7150.f, 1350.f, -11040.f);
			}
			else {
		...
		case LEVEL_CCC:
			vec3f_set(c->pos, -8428.f, -1685.f, 5748.f);
			break;
 ...
 */
void cutscene_non_painting_set_cam_focus(struct Camera *c) { // Manual Camera Adjustment
    ...
    if ((gPrevLevel == LEVEL_COTMC) || (gPrevLevel == LEVEL_HMC) || (gPrevLevel == LEVEL_RR)
        || (gPrevLevel == LEVEL_WMOTR) || (gPrevLevel == LEVEL_TSOBF) || (gPrevLevel == LEVEL_GB) || ((gPrevLevel == LEVEL_BBB) && (gCurrLevelNum != LEVEL_CASTLE_COURTYARD)) || (gPrevLevel == LEVEL_CCC)) {
 ...
 */
void cutscene_exit_painting_start(struct Camera *c) { // Manual Camera Adjustment
    struct Surface *floor;
    f32 floorHeight;
    ...
    if ((gPrevLevel == LEVEL_TTM) || (gPrevLevel == LEVEL_SI) || (gPrevLevel == LEVEL_GB) || (gPrevLevel == LEVEL_BBB) || (gPrevLevel == LEVEL_CCC)) {
 ...
 */
void cutscene_exit_painting(struct Camera *c) { // Manual Camera Adjustment
	...
	} else if (gPrevLevel == LEVEL_SI) {
		vec3f_set(c->pos, 2987.f, 950.f, -3998.f);
	} else if (gPrevLevel == LEVEL_GB) {
		vec3f_set(c->pos, 4532.f, 950.f, -4540.f);
	} else if (gPrevLevel == LEVEL_BBB) {
		vec3f_set(c->pos, 1100.f, 1950.f, 970.f);
	} else if (gPrevLevel == LEVEL_CCC) {
		vec3f_set(c->pos, 3550.f, 1800.f, 3000.f);
	}
...
void cutscene_exit_painting_end(struct Camera *c) {
    ...
    c->mode = c->defMode;
#else
    // Original: c->mode = CAMERA_MODE_CLOSE;
#endif
    c->cutscene = CUTSCENE_NONE;
    gCutsceneTimer = CUTSCENE_STOP;
    sStatusFlags |= CAM_FLAG_SMOOTH_MOVEMENT;
 ...
 *
 * Make sure that level at the end of defined levels is a stub so sZoomOutAreaMasks for real last level can work properly
 */
