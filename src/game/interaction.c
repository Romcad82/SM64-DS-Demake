...
u32 interact_silver_star   (struct MarioState* m, u32 interactType, struct Object* obj);
...
static struct InteractionHandler sInteractionHandlers[] = {
	...
	{ INTERACT_SILVER_STAR,    interact_silver_star },
};
...
u32 take_damage_and_knock_back(struct MarioState *m, struct Object *obj) {
    ...
    if (!sInvulnerable && !(m->flags & MARIO_VANISH_CAP)
		...
		if (cur_obj_nearest_object_with_behavior(bhvSilversHiddenStar)) {
			cur_obj_nearest_object_with_behavior(bhvSilversHiddenStar)->oSpawnSilverStar = TRUE;
		}
...
u32 interact_star_or_key(struct MarioState *m, UNUSED u32 interactType, struct Object *obj) {
    ...
    //
	if (((gCurrLevelNum == LEVEL_GB) && (gCurrAreaIndex == 1)) || ((gCurrLevelNum == LEVEL_BBB) && (gCurrAreaIndex == 1)) || ((gCurrLevelNum == LEVEL_CCC) && (gCurrAreaIndex == 1)) || (gCurrLevelNum == LEVEL_CASTLE_COURTYARD) || (gCurrLevelNum == LEVEL_CASTLE)) { // Custom fix. Remove later if causes trouble
		noExit = TRUE;
	}
    //
...
u32 interact_silver_star(struct MarioState *m, UNUSED u32 interactType, struct Object *obj) {
	u32 starGrabAction = ACT_STAR_DANCE_NO_EXIT;
	...
	if ((m->health >= 0x100) && (obj->oTimer > 10) && (obj->oCollectedWithShell == FALSE)) {

		if (!(m->action & ACT_FLAG_RIDING_SHELL)) {
			mario_stop_riding_and_holding(m);
		} else {
			obj->oCollectedWithShell = TRUE;
		}
		// Original: mario_stop_riding_and_holding(m);
		...
		obj->parentObj->oPlayUpdatedMusic++;

		if (obj->oCollectedWithShell == FALSE) {
			return set_mario_action(m, starGrabAction, noExit + 2 * grandStar);
		} else {
			play_silver_star_music(FALSE, obj->parentObj->oPlayUpdatedMusic);
		}
		// Original: return set_mario_action(m, starGrabAction, noExit + 2 * grandStar);
	}
...
u32 interact_warp_door(struct MarioState *m, UNUSED u32 interactType, struct Object *obj) { 
    ...
    // Original: s16 warpDoorId = (obj->oBehParams >> 24);
#endif
    ...
    if (m->action == ACT_WALKING || m->action == ACT_DECELERATING) {
		...
		if (warpDoorId == 3) { // Never meant to be unlocked
			if (!sDisplayingDoorText) {
				set_mario_action(m, ACT_READING_AUTOMATIC_DIALOG, DIALOG_176);
			}
			sDisplayingDoorText = TRUE;
...
u32 get_door_save_file_flag(struct Object *door) {
    ...
    switch (requiredNumStars) {
        ...
        case  8: saveFileFlag = SAVE_FLAG_UNLOCKED_8_STAR_DOOR;                                          break;
		case 12: saveFileFlag = SAVE_FLAG_UNLOCKED_BITDW_DOOR;                                           break;
...
u32 interact_door(struct MarioState *m, UNUSED u32 interactType, struct Object *obj) {
    ...
    if (m->action == ACT_WALKING || m->action == ACT_DECELERATING) {
        ...
        if (numStars >= requiredNumStars) {
			...
			u32 actionArg;
			if (obj->oBehParams2ndByte == 0x00) {
				actionArg = should_push_or_pull_door(m, obj);
			} else {
				actionArg = should_push_or_pull_door(m, obj) + WARP_FLAG_DOOR_IS_WARP; // This is so door can be warp door and unlock with star custscene
			}
            // Original: u32 actionArg = should_push_or_pull_door(m, obj);
            ...
            switch (requiredNumStars) {
				...
				case 12: text = DIALOG_179 << 16; break;
                case 30: text = DIALOG_027 << 16; break;
                case 50: text = DIALOG_028 << 16; break;
                case 80: text = DIALOG_029 << 16; break;
            }
...
#endif
    } else if (m->action == ACT_IDLE && sDisplayingDoorText == TRUE && requiredNumStars == 80) {
        m->interactObj = obj;
        m->usedObj     = obj;
    ...
    if (interaction & INT_ATTACK_NOT_FROM_BELOW) {
		...
		if ((obj_has_behavior(obj, bhvChiefChilly)) && (obj->oCanDropCoins == TRUE)) {
			if ((random_u16() % (100 * obj->oCoinChanceMult)) == 0) {
				...
				obj->oCanDropCoins = FALSE;
			} else if ((random_u16() % (15 * obj->oCoinChanceMult)) == 0) {
				obj->oNumLootCoins = 3;
				obj_spawn_loot_yellow_coins(obj, obj->oNumLootCoins, 20.0f);
				obj->oCanDropCoins = FALSE;
			} else if ((random_u16() % (10 * obj->oCoinChanceMult)) == 0) {
				obj->oNumLootCoins = 1;
				obj_spawn_loot_yellow_coins(obj, obj->oNumLootCoins, 20.0f);
				obj->oCanDropCoins = FALSE;
			}
...
u32 interact_spiny_walking(struct MarioState *m, UNUSED u32 interactType, struct Object *obj) {
    ...
    if (interaction & INT_PUNCH) {
        ...
        bounce_back_from_attack(m, interaction);
    } else if (interaction & INT_KICK) {
		obj->oInteractStatus = INT_STATUS_INTERACTED | INT_STATUS_WAS_ATTACKED | ATTACK_KICK_OR_TRIP;
		bounce_back_from_attack(m, interaction);
    } else if (take_damage_and_knock_back(m, obj)) {
		return TRUE;
	}
