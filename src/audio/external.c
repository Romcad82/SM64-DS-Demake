...
u8 sDialogSpeaker[] = {
	...
	/*17*/ _,	  BOMB,	 _,		_,	   _,	  _,	 BOWS1,	_,	   _,	  BOWS1,
	/*18*/ _,	  KBOMB, KBOMB, BOO,   BOO,	  _,	 _,     _,     _
};
#undef _
...
u8 sBackgroundMusicDefaultVolume[] = {
	...
	120, // SEQ_LEVEL_SI
	126, // SEQ_EVENT_BOSS_INTRO
	120, // SEQ_EVENT_CUTSCENE_COLLECT_SS1
	120, // SEQ_EVENT_DROP_STAR
	120, // SEQ_EVENT_CUTSCENE_COLLECT_SS2
	120, // SEQ_EVENT_CUTSCENE_COLLECT_SS3
...
void play_silver_star_music(s32 isHit, u8 silverStarCount) {
	if (isHit) {
		seq_player_play_sequence(SEQ_PLAYER_ENV, SEQ_EVENT_DROP_STAR, 0);
	} else {
		switch (silverStarCount) {
			case 2:
				seq_player_play_sequence(SEQ_PLAYER_ENV, SEQ_EVENT_CUTSCENE_COLLECT_SS2, 0);
				break;

			case 3:
				seq_player_play_sequence(SEQ_PLAYER_ENV, SEQ_EVENT_CUTSCENE_COLLECT_SS3, 0);
				break;

			case 4:
				seq_player_play_sequence(SEQ_PLAYER_ENV, SEQ_EVENT_CUTSCENE_COLLECT_SS4, 0);
				break;

			case 5:
				seq_player_play_sequence(SEQ_PLAYER_ENV, SEQ_EVENT_CUTSCENE_COLLECT_SS5, 0);
				break;

			default:
				seq_player_play_sequence(SEQ_PLAYER_ENV, SEQ_EVENT_CUTSCENE_COLLECT_SS1, 0);
				break;
		}
	}
	sBackgroundMusicMaxTargetVolume = TARGET_VOLUME_IS_PRESENT_FLAG | 0;
